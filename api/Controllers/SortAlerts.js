...
const getAdminAlerts = async (req, res) => {
  ...
  try {
    ...
    const assignedPatients = assignedPatientsResult.map(
      (row) => row.patient_id
    );
    ...
    for (const patientId of assignedPatients) {
      ...
      const patientAlerts = await pool.query(
        `SELECT * FROM alerts WHERE patientId = '${patientId}' and missedAlertId IS NULL`
      );
    ...
    for (const alert of alerts) {
      if (
        ...
        (alert.category.toLowerCase().includes("missed") &&
          alert.category != "Missed Alarm")
      ) {
        if (alert) {
    ...
    for (const alert of filteredAlerts) {
      if (alert.category.slice(0, 23).trim() === "Doctor Message to Admin") {
        ...
        let disapprovalAlert = await generatePrescriptionDisapprovalAlert(
          alert
        );
...
const getDoctorAlerts = async (req, res) => {
  ...
  for (var i = 0; i < alerts.length; i++) {
    if (alerts[i].category === "New Prescription Alarm") {
      const palert = await generateNewPrescriptionAlarmAlert(
        alerts[i],
        doctor_id
      );
      if (palert) {
...
module.exports = {
  getAdminAlerts,
  getDoctorAlerts,
  generateNewPrescriptionAlarmAlert,
  generateLabResultsAlert,
  structureReadingAlert,
  getSuperAdminExtraAlerts,
};
