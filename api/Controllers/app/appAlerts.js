...
const insertAlert = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (category === "Consult Doctor") {
        message = `Please consult Dr. ${dname} immediately`;
      } else if (category === "Send Message") {
        message = `Dr. ${dname} says: ${mess}`;
      }
      ...
      await pool.query(insertQuery);
       ...
       const pushNotiData = {
        title: `New Alert: ${category}`,
        body: message,
        type: category, 
        customField: "this is custom noti from firebase :)", 
      };

       try {
        await sendPushNotification(pushNotiData, patientId);
