...
const fetchAnswers = async (req, res) => {
  console.log("fetch answers request");
  try {
    const { userId } = req.body;

    // Fetch ailments associated with the user
    const ailmentsQuery = `SELECT ailment_id FROM ailment_patient WHERE patient_id = ${userId}`;
    const ailments = await pool.execute(ailmentsQuery);

    // Fetch all answers for the user
    const answersQuery = `
      SELECT ur.question_id, ur.response, q.name AS question, q.type AS questionType, q.options, qa.ailment_id
      FROM user_responses ur
      INNER JOIN questions q ON ur.question_id = q.id
      INNER JOIN question_ailments qa ON q.id = qa.question_id
      WHERE ur.user_id = ${userId}`;
    const answers = await pool.execute(answersQuery);

    let transformedData = {};

    // Fetch Generic Profile Answers
    transformedData["Generic Profile"] = {
      ailmentID: 0,
      ailmentName: "Generic Profile",
      questions: answers
        .filter((answer) => answer.ailment_id === 0)
        .map((answer) => ({
    ...
    await Promise.all(
      ailments.map(async (ailment) => {
        ...
        const ailmentAnswers = answers.filter(
          (answer) => answer.ailment_id === ailment_id
        );

        transformedData[ailmentName] = {
          ailmentID: ailment_id,
          ailmentName: ailmentName,
          questions: ailmentAnswers.map((answer) => ({
            questionID: answer.question_id,
            question: answer.question,
            questionType: answer.questionType,
            answer: answer.response,
