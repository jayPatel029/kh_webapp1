  ...
  try {
    ...
    if (type === "Int" || type === "Decimal") {
        ...
        if (ur) {
          ...
          if (color === "red" || color === "yellow") {
            ...
            const data = {
              ...
              Type: dr.type,
              Date: date,
              AlertText: dr.alertTextDoc
                ? dr.alertTextDoc + `${readings}`
                : `The reading for ${dr.title} is ${readings} which is in the ${color} zone`,
              patientId: user_id,
              color: cc,
              dialyordia: "daily",
              questionId: dr.id,
            ...
            try {
              ...
              //! update patients condition!!
              // await UpdatePatientCondition(user_id);
              return {
                success: true,
          ...
          if (inReadings >= hr || inReadings <= lr) {
            const data = {
              ...
              AlertText: dr.alertTextDoc
                ? dr.alertTextDoc + `${readings}`
                : `The reading for ${dr.title} is ${readings} which is in the red zone`,
              patientId: user_id,
              color: "red",
              dialyordia: "daily",
              questionId: dr.id,
      ...
      const data = {
        ...
        AlertText: dr.alertTextDoc
          ? dr.alertTextDoc + `${readings}`
          : `The response for ${dr.title} is ${readings}`,
        patientId: user_id,
        color: "red",
        dialyordia: "daily",
        questionId: dr.id,
...
const AddDialysisReadingsAlerts = async (
  question_id,
  user_id,
  date,
  readings
...
// const UpdatePatientCondition = async (patientId) => {
//   try {
//     const getConditionQuery = `SELECT \`condition\` FROM patients WHERE id = ?`;
//     const [rows] = await pool.query(getConditionQuery, [patientId]);

//     console.log("patients data:", rows.condition);
...
//     let currentCondition = rows.condition;
//     console.log("curr condition:", currentCondition);

//     let newCondition = null;
//     if (currentCondition === "stable") {
//       newCondition = "unstable";
//     } else if (currentCondition === "unstable") {
...
//       console.log(`${patientId} condition updated ${newCondition}`);
//       return { success: true, message: `Condition updated to ${newCondition}` };
//     } else {
//       console.log(`no update req. : ${patientId}`);
//       return { success: true, message: "No condition change required" };
//     }
//   } catch (error) {
